...
def plot_gh_results_manual(weights, estimates, predictions, actual):
    n = len(weights)
    xs = range(n+1)
    ylim1 = 1e8
    ylim2 = -1*1e8
    ...
    ylim2 = ylim2 + r * 0.1
    ylim1 = ylim1 - r * 0.1
#     book_plots.plot_measurements(range(1, n+1), weights, color='k', lines=False)
    book_plots.plot_filter(xs, estimates, marker='o', label='Estimates')
#     book_plots.plot_track(xs[1:], predictions, c='r', marker='v', label='Predictions')
    plt.plot(xs, actual, c='k', lw=1, label='Actual')
    plt.legend(loc=4)
    book_plots.set_labels(x='time step', y='location')
    plt.xlim([-1, n+1])
    plt.ylim([ylim1, ylim2])
    
def plot_gh_results_manual1(weights, estimates, predictions, actual):
    n = len(weights)
    xs = range(n+1)
    ylim1 = 1e8
    ylim2 = -1*1e8
...
def plot_gh_results_manual2(weights, estimates, predictions, actual):
    n = len(weights)
    xs = range(n+1)
    ylim1 = 1e8
    ylim2 = -1*1e8
